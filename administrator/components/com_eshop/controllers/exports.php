	...
	function _exportProducts()
	{
		...
		if (count($rows))
		{
			...
			if ($exportFormat == 'csv' || $exportFormat == 'xlsx')
			{
				...
				$resultsArr[] = 'product_cost';
				$resultsArr[] = 'product_price';
				$resultsArr[] = 'product_call_for_price';
				$resultsArr[] = 'product_taxclass_id';
				$resultsArr[] = 'product_manage_stock';
				$resultsArr[] = 'product_stock_display';
				$resultsArr[] = 'product_stock_warning';
				$resultsArr[] = 'product_inventory_global';
				...
				$resultsArr[] = 'product_shipping_cost_geozones';
			...
			foreach ($rows as $row)
			{
				...
				if ($exportFormat == 'csv' || $exportFormat == 'xlsx')
				{
					...
					$resultsArr[] = $row->product_cost;
					$resultsArr[] = $row->product_price;
					$resultsArr[] = $row->product_call_for_price;
					$resultsArr[] = $row->product_taxclass_id;
					$resultsArr[] = $row->product_manage_stock;
					$resultsArr[] = $row->product_stock_display;
					$resultsArr[] = $row->product_stock_warning;
					$resultsArr[] = $row->product_inventory_global;
					...
					$resultsArr[] = $row->product_shipping_cost_geozones;
					...
					$resultsArr[] = $row->product_cart_mode;
					...
					if (EshopHelper::getConfigValue('product_custom_fields'))
					{
					    foreach ($row->paramData as $param)
					    {
					        $resultsArr[] = $param['value'];
					    }   
					}
					$csvOutput[] = $resultsArr;
				}
				else 
				{
					...
					if (EshopHelper::getConfigValue('product_custom_fields'))
					{
					    foreach ($row->paramData as $key => $param)
					    {
					        $productArray = array_merge($productArray, array($key => $param['value']));
					    }
					}
			...
			{
			    require_once JPATH_ADMINISTRATOR . '/components/com_eshop/libraries/vendor/xlsxwriter/xlsxwriter.class.php';
			    ...
			    $writer->writeSheet($csvOutput, 'Sheet1');
		    ...
		    elseif ($exportFormat == 'xml')
		    {
		        ...
		        foreach ($rows as $row)
		        {
		            ...
		            foreach ($productArray as $value)
		            {
		                ...
		                if ($googlePrice)
		                {
		                    $productArray1['g:price'] = number_format($row->product_price * 1.19, 2) . ' ' . $defaultCurrency;
		                }
		                if ($googleSalePrice)
		                {
		                    $productArray1['g:sale_price'] = number_format($row->product_price * 1.19, 2) . ' ' . $defaultCurrency;
		                }
		                if ($googleMpn)
		                {
		        ...
		        $productXmlArray[0]['@attributes'] = array('version' => '2.0', 'xmlns:g' => 'http://base.google.com/ns/1.0');
		        
		        $filename = 'google_feed_'.date('YmdHis').'.xml';
		        include_once JPATH_ROOT.'/components/com_eshop/helpers/array2xml.php';
		        $xml = Array2XML::createXML('rss', $productXmlArray[0]);
		        JFile::write(JPATH_ROOT . '/' . $filename, $xml->saveXML());
