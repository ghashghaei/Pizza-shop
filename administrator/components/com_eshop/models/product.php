...
class EShopModelProduct extends EShopModel
{
	...
	function store(&$data)
	{
	    ...
	    {
	        $mainframe->enqueueMessage(JText::_('ESHOP_ERROR_PRODUCT_SKU_EXISTED'), 'error');
		...
		$app->triggerEvent('onAfterSaveProduct', array($row, $data, $isNew));
		...
		if (count($productOptionId))
		{
			...
			for ($i = 0; $n = count($productOptionId), $i < $n; $i++)
			{
				...
				$optionValueImages = $_FILES['optionvalue_' . $optionId . '_image'];
				$optionValueImage  = $input->getString('optionvalue_' . $optionId . '_imageold');
