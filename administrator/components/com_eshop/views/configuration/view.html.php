...
class EShopViewConfiguration extends JViewLegacy
{
	...
	public function display($tpl = null)
	{
		...
		if (JFile::exists(JPATH_ADMINISTRATOR . '/components/com_acymailing/helpers/helper.php'))
		{
		    ...
		    $listIds                      = explode(',', isset($tempConfig->acymailing_list_ids) ? $tempConfig->acymailing_list_ids : '');
		...
		$options							        = array();
		$options[]							        = JHtml::_('select.option', 'payment_method_step', JText::_('ESHOP_PAYMENT_METHOD_STEP'));
		$options[]							        = JHtml::_('select.option', 'confirm_step', JText::_('ESHOP_CONFIRM_STEP'));
		$lists['display_privacy_policy']			= JHtml::_('select.genericlist', $options, 'display_privacy_policy', ' class="inputbox" ', 'value', 'text', isset($config->display_privacy_policy) ? $config->display_privacy_policy : 'step_5');
		...
		//Build products filter layout list
		$options							        = array();
		$options[]							        = JHtml::_('select.option', 'popout', JText::_('ESHOP_CART_POPOUT'));
		$options[]							        = JHtml::_('select.option', 'redirect', JText::_('ESHOP_CART_REDIRECT'));
		$options[]							        = JHtml::_('select.option', 'message', JText::_('ESHOP_CART_MESSAGE'));
		$lists['cart_popout']						= JHtml::_('select.genericlist', $options, 'cart_popout', ' class="inputbox" ', 'value', 'text', isset($config->cart_popout) ? $config->cart_popout : 'popout');
		...
		$lists['product_sku_validation']			= EshopHtmlHelper::getBooleanInput('product_sku_validation', isset($config->product_sku_validation) ? $config->product_sku_validation : '0');
		...
		$lists['dynamic_info']						= EshopHtmlHelper::getBooleanInput('dynamic_info', isset($config->dynamic_info) ? $config->dynamic_info : '0');
		...
		$options[]                                  = JHtml::_('select.option', 'registered', 'Only Registered Users');
		...
		$lists['display_option_price_with_tax']		= EshopHtmlHelper::getBooleanInput('display_option_price_with_tax', isset($config->display_option_price_with_tax) ? $config->display_option_price_with_tax : '1');
		...
		$lists['shop_offline']						= EshopHtmlHelper::getBooleanInput('shop_offline', isset($config->shop_offline) ? $config->shop_offline : '0');
		...
		$lists['checkout_weight']					= EshopHtmlHelper::getBooleanInput('checkout_weight', isset($config->checkout_weight) ? $config->checkout_weight : '0');
		...
		$lists['enable_existing_addresses']			= EshopHtmlHelper::getBooleanInput('enable_existing_addresses', isset($config->enable_existing_addresses) ? $config->enable_existing_addresses : '1');
		...
		$lists['stock_manage']						= EshopHtmlHelper::getBooleanInput('stock_manage', isset($config->stock_manage) ? $config->stock_manage : '1');
		...
		$options                                    = array();
		$options[]                                  = JHtml::_('select.option', '2:10', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_2_10'));
		$options[]                                  = JHtml::_('select.option', '3:9', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_3_9'));
		$options[]                                  = JHtml::_('select.option', '4:8', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_4_8'));
		$options[]                                  = JHtml::_('select.option', '5:7', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_5_7'));
		$options[]                                  = JHtml::_('select.option', '6:6', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_6_6'));
		$options[]                                  = JHtml::_('select.option', '7:5', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_7_5'));
		$options[]                                  = JHtml::_('select.option', '8:4', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_8_4'));
		$options[]                                  = JHtml::_('select.option', '9:3', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_9_3'));
		$options[]                                  = JHtml::_('select.option', '10:2', JText::_('ESHOP_GRID_RATIO_IMAGE_INFO_10_2'));
		$lists['grid_ratio_image_info']             = JHtml::_('select.genericlist', $options, 'grid_ratio_image_info', ' class="inputbox" ', 'value', 'text', isset($config->grid_ratio_image_info) ? $config->grid_ratio_image_info : '4:8');
		...
		if (EshopHelper::getConfigValue('product_custom_fields'))
		{
		    ...
		    foreach ($fields as $field)
		    {
                ...
                $lists['table_show_'. $name] = EshopHtmlHelper::getBooleanInput('table_show_'. $name, isset($config->{'table_show_'. $name}) ? $config->{'table_show_'. $name} : '1');
		    }
		}
		...
		$lists['invoice_enable']					= EshopHtmlHelper::getBooleanInput('invoice_enable', isset($config->invoice_enable) ? $config->invoice_enable : '0');
		$lists['always_generate_invoice']			= EshopHtmlHelper::getBooleanInput('always_generate_invoice', isset($config->always_generate_invoice) ? $config->always_generate_invoice : '0');
		$lists['send_invoice_to_customer']			= EshopHtmlHelper::getBooleanInput('send_invoice_to_customer', isset($config->send_invoice_to_customer) ? $config->send_invoice_to_customer : '0');
		$lists['send_invoice_to_admin']				= EshopHtmlHelper::getBooleanInput('send_invoice_to_admin', isset($config->send_invoice_to_admin) ? $config->send_invoice_to_admin : '0');
		...
		$options   = array();
		$options[] = JHtml::_('select.option', 'absolutely', JText::_('ESHOP_PDF_IMAGE_PATH_ABSOLUTELY'));
		$options[] = JHtml::_('select.option', 'relatively', JText::_('ESHOP_PDF_IMAGE_PATH_RELATIVELY'));
		
		$lists['pdf_image_path'] 					= JHtml::_('select.genericlist', $options, 'pdf_image_path', ' class="inputbox"', 'value', 'text', isset($config->pdf_image_path) ? $config->pdf_image_path : 'absolutely');
		
		$lists['product_image_rollover']			= EshopHtmlHelper::getBooleanInput('product_image_rollover', isset($config->product_image_rollover) ? $config->product_image_rollover : '0');
		...
		$lists['debug_mode']                   	= EshopHtmlHelper::getBooleanInput('debug_mode', isset($config->debug_mode) ? $config->debug_mode : '0');
		...
		$sortTexts = array(
		    ...
		    JText::_('ESHOP_CONFIG_SORTING_PRODUCT_AVAILABLE_DATE_ASC'),
		    JText::_('ESHOP_CONFIG_SORTING_PRODUCT_AVAILABLE_DATE_DESC'),
		...
		$lists['default_sorting'] = JHtml::_('select.genericlist', $options, 'default_sorting', ' class="inputbox" style="width: 250px;" ', 'value', 'text', isset($config->default_sorting) ? $config->default_sorting : 'a.ordering-ASC');
		
		//Category sort options list
		$options = array();
		$options[]    = JHtml::_('select.option', 'name-asc', JText::_('ESHOP_CONFIG_SORTING_CATEGORY_NAME_ASC'));
		$options[]    = JHtml::_('select.option', 'name-desc', JText::_('ESHOP_CONFIG_SORTING_CATEGORY_NAME_DESC'));
		$options[]    = JHtml::_('select.option', 'ordering-asc', JText::_('ESHOP_CONFIG_SORTING_CATEGORY_ORDERING_ASC'));
		$options[]    = JHtml::_('select.option', 'ordering-desc', JText::_('ESHOP_CONFIG_SORTING_CATEGORY_ORDERING_DESC'));
		$options[]    = JHtml::_('select.option', 'id-asc', JText::_('ESHOP_CONFIG_SORTING_CATEGORY_LATEST'));
		$options[]    = JHtml::_('select.option', 'id-desc', JText::_('ESHOP_CONFIG_SORTING_CATEGORY_OLDEST'));
		$options[]    = JHtml::_('select.option', 'random', JText::_('ESHOP_CONFIG_SORTING_RANDOMLY'));
		$lists['category_default_sorting'] = JHtml::_('select.genericlist', $options, 'category_default_sorting', 'class="inputbox" style="width: 250px;" ', 'value', 'text', isset($config->category_default_sorting) ? $config->category_default_sorting : 'name-asc');
		
		//Manufacturer sort options list
		$options = array();
		$options[]    = JHtml::_('select.option', 'name-asc', JText::_('ESHOP_CONFIG_SORTING_MANUFACTURER_NAME_ASC'));
		$options[]    = JHtml::_('select.option', 'name-desc', JText::_('ESHOP_CONFIG_SORTING_MANUFACTURER_NAME_DESC'));
		$options[]    = JHtml::_('select.option', 'ordering-asc', JText::_('ESHOP_CONFIG_SORTING_MANUFACTURER_ORDERING_ASC'));
		$options[]    = JHtml::_('select.option', 'ordering-desc', JText::_('ESHOP_CONFIG_SORTING_MANUFACTURER_ORDERING_DESC'));
		$options[]    = JHtml::_('select.option', 'id-asc', JText::_('ESHOP_CONFIG_SORTING_MANUFACTURER_LATEST'));
		$options[]    = JHtml::_('select.option', 'id-desc', JText::_('ESHOP_CONFIG_SORTING_MANUFACTURER_OLDEST'));
		$options[]    = JHtml::_('select.option', 'random', JText::_('ESHOP_CONFIG_SORTING_RANDOMLY'));
		$lists['manufacturer_default_sorting'] = JHtml::_('select.genericlist', $options, 'manufacturer_default_sorting', 'class="inputbox" style="width: 250px;" ', 'value', 'text', isset($config->manufacturer_default_sorting) ? $config->manufacturer_default_sorting : 'name-asc');
		
		//Option sort options list
		$options = array();
		$options[]    = JHtml::_('select.option', 'name-asc', JText::_('ESHOP_CONFIG_SORTING_OPTION_NAME_ASC'));
		$options[]    = JHtml::_('select.option', 'name-desc', JText::_('ESHOP_CONFIG_SORTING_OPTION_NAME_DESC'));
		$options[]    = JHtml::_('select.option', 'ordering-asc', JText::_('ESHOP_CONFIG_SORTING_OPTION_ORDERING_ASC'));
		$options[]    = JHtml::_('select.option', 'ordering-desc', JText::_('ESHOP_CONFIG_SORTING_OPTION_ORDERING_DESC'));
		$options[]    = JHtml::_('select.option', 'id-asc', JText::_('ESHOP_CONFIG_SORTING_OPTION_LATEST'));
		$options[]    = JHtml::_('select.option', 'id-desc', JText::_('ESHOP_CONFIG_SORTING_OPTION_OLDEST'));
		$options[]    = JHtml::_('select.option', 'random', JText::_('ESHOP_CONFIG_SORTING_RANDOMLY'));
		$lists['option_default_sorting'] = JHtml::_('select.genericlist', $options, 'option_default_sorting', 'class="inputbox" style="width: 250px;" ', 'value', 'text', isset($config->option_default_sorting) ? $config->option_default_sorting : 'name-asc');
		
		//Option value sort options list
		$options = array();
		$options[]    = JHtml::_('select.option', 'name-asc', JText::_('ESHOP_CONFIG_SORTING_OPTION_VALUE_NAME_ASC'));
		$options[]    = JHtml::_('select.option', 'name-desc', JText::_('ESHOP_CONFIG_SORTING_OPTION_VALUE_NAME_DESC'));
		$options[]    = JHtml::_('select.option', 'ordering-asc', JText::_('ESHOP_CONFIG_SORTING_OPTION_VALUE_ORDERING_ASC'));
		$options[]    = JHtml::_('select.option', 'ordering-desc', JText::_('ESHOP_CONFIG_SORTING_OPTION_VALUE_ORDERING_DESC'));
		$options[]    = JHtml::_('select.option', 'id-asc', JText::_('ESHOP_CONFIG_SORTING_OPTION_VALUE_LATEST'));
		$options[]    = JHtml::_('select.option', 'id-desc', JText::_('ESHOP_CONFIG_SORTING_OPTION_VALUE_OLDEST'));
		$options[]    = JHtml::_('select.option', 'random', JText::_('ESHOP_CONFIG_SORTING_RANDOMLY'));
		$lists['option_value_default_sorting'] = JHtml::_('select.genericlist', $options, 'option_value_default_sorting', 'class="inputbox" style="width: 250px;" ', 'value', 'text', isset($config->option_value_default_sorting) ? $config->option_value_default_sorting : 'name-asc');
		
		//Attribute sort options list
		$options = array();
		$options[]    = JHtml::_('select.option', 'name-asc', JText::_('ESHOP_CONFIG_SORTING_ATTRIBUTE_NAME_ASC'));
		$options[]    = JHtml::_('select.option', 'name-desc', JText::_('ESHOP_CONFIG_SORTING_ATTRIBUTE_NAME_DESC'));
		$options[]    = JHtml::_('select.option', 'ordering-asc', JText::_('ESHOP_CONFIG_SORTING_ATTRIBUTE_ORDERING_ASC'));
		$options[]    = JHtml::_('select.option', 'ordering-desc', JText::_('ESHOP_CONFIG_SORTING_ATTRIBUTE_ORDERING_DESC'));
		$options[]    = JHtml::_('select.option', 'id-asc', JText::_('ESHOP_CONFIG_SORTING_ATTRIBUTE_LATEST'));
		$options[]    = JHtml::_('select.option', 'id-desc', JText::_('ESHOP_CONFIG_SORTING_ATTRIBUTE_OLDEST'));
		$options[]    = JHtml::_('select.option', 'random', JText::_('ESHOP_CONFIG_SORTING_RANDOMLY'));
		$lists['attribute_default_sorting'] = JHtml::_('select.genericlist', $options, 'attribute_default_sorting', 'class="inputbox" style="width: 250px;" ', 'value', 'text', isset($config->attribute_default_sorting) ? $config->attribute_default_sorting : 'name-asc');
		
		//Image
		$options                                   = array();
		$options[]                                 = JHtml::_('select.option', 'notResizeImage', JText::_('ESHOP_NOT_RESIZE_IMAGE'));
		$options[]                                 = JHtml::_('select.option', 'resizeImage', JText::_('ESHOP_RESIZE_IMAGE'));
		$options[]                                 = JHtml::_('select.option', 'cropsizeImage', JText::_('ESHOP_CROPSIZE_IMAGE'));
		$options[]                                 = JHtml::_('select.option', 'maxsizeImage', JText::_('ESHOP_MAXSIZE_IMAGE'));
