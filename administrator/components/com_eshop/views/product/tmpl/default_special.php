...
<table class="adminlist table table-bordered" style="text-align: center;">
	<thead>
	<tr>
		<th class="text_center" width="10%"><?php echo JText::_('ESHOP_CUSTOMER_GROUP'); ?></th>
		...
		<th class="text_center" width="25%"><?php echo JText::_('ESHOP_START_DATE'); ?></th>
		<th class="text_center" width="25%"><?php echo JText::_('ESHOP_END_DATE'); ?></th>
	...
	for ($i = 0; $n = count($this->productSpecials), $i < $n; $i++) {
		...
		<tr id="product_special_<?php echo $i; ?>">
			...
			<td style="text-align: center;">
				<?php echo JHtml::_('calendar', (($productSpecial->date_start == $this->nullDate) || !$productSpecial->date_start) ? '' : JHtml::_('date', $productSpecial->date_start, 'Y-m-d H:i', null), 'special_date_start[]', 'special_date_start_'.$i, '%Y-%m-%d %H:%M', array('class' => 'input-medium')); ?>
			</td>
			<td style="text-align: center;">
				<?php echo JHtml::_('calendar', (($productSpecial->date_end == $this->nullDate) || !$productSpecial->date_end) ? '' : JHtml::_('date', $productSpecial->date_end, 'Y-m-d H:i', null), 'special_date_end[]', 'special_date_end_'.$i, '%Y-%m-%d %H:%M', array('class' => 'input-medium')); ?>
			</td>
			<td style="text-align: center;">
