	...
	<div id="j-main-container">
		<div id="filter-bar" class="btn-toolbar">
			<div class="filter-search btn-group pull-left">
				<label for="filter_search" class="element-invisible"><?php echo JText::_('ESHOP_FILTER_SEARCH_TAXCLASSES_DESC');?></label>
				<input type="text" name="search" id="filter_search" placeholder="<?php echo JText::_('JSEARCH_FILTER'); ?>" value="<?php echo $this->escape($this->state->search); ?>" class="hasTooltip" title="<?php echo JHtml::tooltipText('ESHOP_SEARCH_TAXCLASSES_DESC'); ?>" />
			</div>
		...
		<table class="adminlist table table-striped">
			...
			for ($i=0, $n=count( $this->items ); $i < $n; $i++)
			{
				...
				$published 	= JHtml::_('jgrid.published', $row->published, $i, 'taxclass.');
				?>
				<tr class="<?php echo "row$k"; ?>">
					...
					<td class="text_center">
						<?php echo JHtml::_('date', $row->created_date, $dateFormat, null); ?>
					</td>
					<td class="text_center">
						<?php echo JHtml::_('date', $row->modified_date, $dateFormat, null); ?>
					</td>
					<td class="text_center">
						<?php echo $published; ?>
					</td>
