	...
	public static function getFilterData()
	{
		...
		if ($filterData === null)
		{
			...
			if ($option == 'com_eshop' && $view == 'manufacturer')
			{
			    ...
			    $manufacturerIds = array($manufacturerId);
			}
			else 
			{
	...
	public static function getCategories($filterData = array())
	{
		...
		if (count($rows))
		{
			...
			foreach ($rows as $row)
			{
				...
				$query->where('((p.product_customergroups = "") OR (p.product_customergroups IS NULL) OR (p.product_customergroups = "' . $customerGroupId . '") OR (p.product_customergroups LIKE "' . $customerGroupId . ',%") OR (p.product_customergroups LIKE "%,' . $customerGroupId . ',%") OR (p.product_customergroups LIKE "%,' . $customerGroupId . '"))');
