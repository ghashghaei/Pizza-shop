...
?>
<div class="cart-info">
	<table class="table table-bordered uk-table-responsive uk-table uk-table-hover">
		<thead>
			<tr>
		...
		<tbody>
			...
			foreach ($this->totalData as $data)
			{
				?>
				<tr>
					<td colspan="4" class="uk-text-left uk-text-middle"><?php echo $data['title']; ?>:</td>
					<td class="uk-text-right uk-text-success uk-text-middle"><strong><?php echo $data['text']; ?></strong></td>
				</tr>
				<?php	
			}
			?>
...
else 
{
	...
	<form action="<?php echo EshopHelper::getSiteUrl(); ?>index.php?option=com_eshop&task=checkout.processOrder&Itemid=<?php echo EshopRoute::getDefaultItemId(); ?>" method="post" name="payment_method_form" id="payment_method_form" class="form form-horizontal">
		...
		<div class="no_margin_left">
			<div class="no_margin_left">
				<input id="button-confirm" type="button" onclick="validateCheckoutData();" class="uk-button uk-button-primary" value="<?php echo JText::_('ESHOP_CONFIRM_ORDER'); ?>" />
			</div>
		</div>
