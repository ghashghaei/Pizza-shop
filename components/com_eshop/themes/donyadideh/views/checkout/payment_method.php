...
if (EshopHelper::getConfigValue('enable_checkout_donate'))
{
	...
	<div>
		...
		else 
		{
			?>
			<div class="uk-margin">
			<input type="text" name="other_amount" id="other_amount" class="uk-input uk-form-width-large" placeholder="<?php echo JText::_('ESHOP_DONATE_AMOUNT'); ?>" />
			</div>
			<?php
		}
		?>
...
if (EshopHelper::getConfigValue('allow_coupon'))
{
	?>
	<div class="uk-margin">
			<input type="text" id="coupon_code" name="coupon_code" class="uk-input uk-form-width-large" value="<?php echo htmlspecialchars($this->coupon_code, ENT_COMPAT, 'UTF-8'); ?>" placeholder="<?php echo JText::_('ESHOP_COUPON_TEXT'); ?>">
	</div>
	<?php
}
if (EshopHelper::getConfigValue('allow_voucher'))
{
	?>
	<div class="uk-margin">
			<input type="text" id="voucher_code" name="voucher_code" class="uk-input uk-form-width-large" value="<?php echo htmlspecialchars($this->voucher_code, ENT_COMPAT, 'UTF-8'); ?>" placeholder="<?php echo JText::_('ESHOP_VOUCHER_TEXT'); ?>">
	</div>
	<?php
}
?>
<div class="uk-margin">
		<textarea rows="5" id="textarea" class="uk-textarea uk-form-width-large" name="comment" placeholder="<?php echo JText::_('ESHOP_COMMENT_ORDER'); ?>"><?php echo $this->comment; ?></textarea>
</div>
<?php
...
<div class="uk-margin uk-float-left">
	<input type="button" class="<?php echo $btnClass; ?> btn-primary <?php echo $pullRightClass; ?>" id="button-payment-method" value="<?php echo JText::_('ESHOP_CONTINUE'); ?>" />
</div>
</div>
<script type="text/javascript">
		...
		$('#button-payment-method').click(function(){
			...
			$.ajax({
				...
				beforeSend: function() {
					...
					$('#button-payment-method').after('<span class="wait">&nbsp;<img src="components/com_eshop/assets/images/loading.svg" alt="" /></span>');
				},	
				complete: function() {
