...
?>
<h3><?php echo JText::_('ESHOP_PRODUCT_COMPARE'); ?></h3>
<?php
...
<div class="uk-overflow-auto">
	<table class="table table-bordered uk-table-striped uk-table uk-table-hover">
		...
		<tbody>
			<tr>
				<td width="20%" class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_PRODUCT'); ?></b></td>
				<?php
			...
			if (EshopHelper::getConfigValue('compare_image'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_IMAGE'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						$viewProductUrl = JRoute::_(EshopRoute::getProductRoute($product['product_id'], EshopHelper::getProductCategory($product['product_id'])));
			...
			if (EshopHelper::showPrice() && EshopHelper::getConfigValue('compare_price'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_PRICE'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (EshopHelper::getConfigValue('compare_sku'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_MODEL'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (EshopHelper::getConfigValue('compare_manufacturer'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_BRAND'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (EshopHelper::getConfigValue('compare_availability'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_AVAILABILITY'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (EshopHelper::getConfigValue('compare_rating'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_RATING'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
						<td>
							<img src="components/com_eshop/assets/images/stars-<?php echo round($product['rating']); ?>.png" />
							<small><?php echo sprintf(JText::_('ESHOP_COMPARE_NUM_REVIEWS'), $product['num_reviews']); ?></small>
						</td>
						<?php
					}
					?>
			...
			if (EshopHelper::getConfigValue('compare_short_desc'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_SHORT_DESCRIPTION'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (EshopHelper::getConfigValue('compare_desc'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_DESCRIPTION'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (EshopHelper::getConfigValue('compare_weight'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_WEIGHT'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (EshopHelper::getConfigValue('compare_dimensions'))
			{
				?>
				<tr>
					<td class="uk-text-left"><b><?php echo JText::_('ESHOP_COMPARE_DIMENSIONS'); ?></b></td>
					<?php
					foreach ($this->products as $product)
					{
						?>
			...
			if (count($this->visibleAttributeGroups) && EshopHelper::getConfigValue('compare_attributes'))
			{
				foreach ($this->visibleAttributeGroups as $key => $visibleAttributeGroup)
				{
					...
					foreach ($visibleAttributeGroup['attribute_name'] as $attributeName)
					{
						?>
						<tr>
							<td class="uk-text-left"><?php echo $attributeName; ?></td>
							<?php
							foreach ($this->products as $product)
							{
								?>
			...
			<tr>
				...
				foreach ($this->products as $product)
				{
					?>
					<td style="text-align:center;">
						<input type="button" class="uk-button uk-button-danger uk-button-small" onclick="removeFromCompare(<?php echo $product['product_id']; ?>, '<?php echo EshopHelper::getSiteUrl(); ?>', '<?php echo EshopHelper::getAttachedLangLink(); ?>');" value="<?php echo JText::_('ESHOP_REMOVE'); ?>" />
					</td>
					<?php
				}
				?>
