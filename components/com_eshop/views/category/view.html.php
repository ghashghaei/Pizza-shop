	...
	public function display($tpl = null)
	{
		...
		else
		{
			...
			$sortTexts = array(
			    ...
			    JText::_('ESHOP_SORTING_PRODUCT_AVAILABLE_DATE_ASC'),
			    JText::_('ESHOP_SORTING_PRODUCT_AVAILABLE_DATE_DESC'),
			...
			if (count($options) > 1)
			{
				$this->sort_options = JHtml::_('select.genericlist', $options, 'sort_options', 'class="inputbox input-xlarge" onchange="this.form.submit();" ', 'value', 'text', $state->sort_options ? $state->sort_options : EshopHelper::getDefaultSortingCategory($state->id));
			}
			else
			{
				$this->sort_options = '';
			...
			$app->setUserState('sort_options', $state->sort_options ? $state->sort_options : EshopHelper::getDefaultSortingCategory($state->id));
			...
			$category->category_desc   = JHtml::_('content.prepare', $category->category_desc);
