...
class EShopViewProduct extends EShopView
{
	...
	public function display($tpl = null)
	{
		...
		else
		{
			...
			{
			    $item->small_thumb_image = $baseUri . '/media/com_eshop/products/' . $smallThumbImage;
			}
			else 
			{
			...
			{
			    $item->thumb_image       = $baseUri . '/media/com_eshop/products/' . $thumbImage;
			}
			else 
			{
			...
			{
			    $item->popup_image       = $baseUri . '/media/com_eshop/products/' . $popupImage;
			}
			else 
			{
			...
			$plugins = JFactory::getApplication()->triggerEvent('onProductDisplay', array($item));
			...
			for ($i = 0; $n = count($productImages), $i < $n; $i++)
			{
				...
				if ($additionalImageSizeFunction == 'notResizeImage')
				{
				    $productImages[$i]->small_thumb_image = JUri::base(true) . '/media/com_eshop/products/' . $smallThumbImage;
				}
				else 
				{
				...
				if ($thumbImageSizeFunction == 'notResizeImage')
				{
				    $productImages[$i]->thumb_image       = JUri::base(true) . '/media/com_eshop/products/' . $thumbImage;
				}
				else 
				{
				...
				if ($popupImageSizeFunction == 'notResizeImage')
				{
				    $productImages[$i]->popup_image       = JUri::base(true) . '/media/com_eshop/products/' . $popupImage;
				}
				else 
				{
			...
			for ($i = 0; $n = count($productRelations), $i < $n; $i++)
			{
				if ($productRelations[$i]->product_image && JFile::exists(JPATH_ROOT . '/media/com_eshop/products/' . $productRelations[$i]->product_image))
				{
					...
					$thumbImage = call_user_func_array(array('EshopHelper', $relatedImageSizeFunction), array($productImage, JPATH_ROOT . '/media/com_eshop/products/', $imageRelatedWidth, $imageRelatedHeight));
				}
				else
				{
					$thumbImage = call_user_func_array(array('EshopHelper', $relatedImageSizeFunction), array('no-image.png', JPATH_ROOT . '/media/com_eshop/products/', $imageRelatedWidth, $imageRelatedHeight));
				}

				if ($relatedImageSizeFunction == 'notResizeImage')
				{
				    $productRelations[$i]->thumb_image = JUri::base(true) . '/media/com_eshop/products/' . $thumbImage;
				}
				else
				{
				...
				if (count($relatedProductImages) > 0)
				{
				    ...
				    if ($relatedProductImage && JFile::exists(JPATH_ROOT . '/media/com_eshop/products/' . $relatedProductImage))
				    {
				        if (EshopHelper::getConfigValue('product_use_image_watermarks'))
				        {
				            $watermarkImage = EshopHelper::generateWatermarkImage(JPATH_ROOT . '/media/com_eshop/products/' . $relatedProductImage);
				            $relatedProductImage   = $watermarkImage;
				        }
				
				        $additionalRelatedProductImage = call_user_func_array(array('EshopHelper', $relatedImageSizeFunction), array($relatedProductImage, JPATH_ROOT . '/media/com_eshop/products/', $imageRelatedWidth, $imageRelatedHeight));
				    }
				    else
				    {
				        $additionalRelatedProductImage = call_user_func_array(array('EshopHelper', $relatedImageSizeFunction), array('no-image.png', JPATH_ROOT . '/media/com_eshop/products/', $imageRelatedWidth, $imageRelatedHeight));
				    }
				
				    if ($relatedImageSizeFunction == 'notResizeImage')
				    {
				        $productRelations[$i]->additional_image = $baseUri . '/media/com_eshop/products/' . $additionalRelatedProductImage;
				    }
				    else
				    {
				        $productRelations[$i]->additional_image = $baseUri . '/media/com_eshop/products/resized/' . $additionalRelatedProductImage;
				    }
				}
			}
			...
			$ratingHtml = '<span class="uk-margin-small-right">' . JText::_('ESHOP_BAD') . '</span>';

			for ($i = 1; $i <= 5; $i++)
			{
				$ratingHtml .= '<input class="uk-radio" type="radio" name="rating" value="' . $i . '" />';
			}

			$ratingHtml .= '<span class="uk-margin-small-left">' . JText::_('ESHOP_EXCELLENT') . '</span>';
