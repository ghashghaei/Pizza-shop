...
abstract class File
{
    ...
    public static function glob($pattern)
    {
        ...
        if (defined('GLOB_BRACE') && !Str::startsWith($pattern, '{')) {
            return glob($pattern, GLOB_BRACE | GLOB_NOSORT);
        }

        return static::_glob($pattern);
    }
     ...
     * @return array
     */
    protected static function _glob($pattern)
    {
        $files = [];

        foreach (Str::expandBraces($pattern) as $file) {
            $files = array_merge($files, glob($file, GLOB_NOSORT) ?: []);
        }
